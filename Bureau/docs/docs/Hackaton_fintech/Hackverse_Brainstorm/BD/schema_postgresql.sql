...
-- SUJET 10 HACKATHON — TWIST 01 & VERSION 2 (MULTI-TENANT)
-- Compatible PostgreSQL 14+
-- ============================================================

DROP VIEW IF EXISTS v_tresorerie_mensuelle CASCADE;
DROP VIEW IF EXISTS v_solde_comptes CASCADE;
DROP TABLE IF EXISTS exports_bilan CASCADE;
...
JOIN comptes cp ON t.compte_id = cp.id
WHERE t.statut = 'CONFIRME'
GROUP BY mois, c.id, c.code, c.groupe_bilan, c.libelle_bilan, t.type_flux, cp.utilisateur_id
...
WHERE cp.actif = TRUE
GROUP BY cp.id, cp.utilisateur_id, cp.nom, cp.type_compte, cp.operateur, cp.devise, cp.solde_initial;
...
ALTER TABLE pieces_jointes 
    DROP CONSTRAINT pieces_jointes_type_fichier_check;
